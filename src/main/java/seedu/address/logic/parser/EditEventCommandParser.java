    ...
    public EditEventCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_DATE, PREFIX_VENUE,
                        PREFIX_START_TIME, PREFIX_TAG);

        EditEventCommand.EditEventDetails editEventDetails = new EditEventCommand.EditEventDetails();
        if (argMultimap.getValue(PREFIX_NAME).isPresent()) {
            editEventDetails.setEventName(ParserUtil.parseEventName(argMultimap.getValue(PREFIX_NAME).get()));
        }
        if (argMultimap.getValue(PREFIX_DATE).isPresent()) {
            editEventDetails.setEventDate(ParserUtil.parseEventDate(argMultimap.getValue(PREFIX_DATE).get()));
        }
        ...
        if (!editEventDetails.isAnyFieldEdited()) {
            throw new ParseException(EditEventCommand.MESSAGE_NOT_EDITED_EVENT);
        }
