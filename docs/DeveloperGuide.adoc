...
The addTag/removeTag mechanism is facilitated by `AddressBook` as it provides an editable form of a `ReadOnlyAddressBook` using the `Model` interface.
...
Figure 12 and figure 13 below are sequence diagrams that summarize the flow of actions when the system performs the addTag() or removeTag() command respectively:

[col="2*"]
|====
    ^| image:AddTagSequenceDiagram.png[] Figure 12 - AddTag Sequence Diagram ^| image:RemoveTagSequenceDiagram.png[] Figure 13 - RemoveTag Sequence Diagram
|====
