...
image::EmailAllClassDiagram.png[width="600"]
...
The figure above represents an activity diagram when the _email_ command is executed. As you can see, the command involves *error checking*, followed by *creating an email object*, and finally *sending* this email to an individual guest.
...
.. Test case: `email 1`+
   Expected: Assuming there is at least one guest, UI Window opens to allow user input. If all inputs are valid, email is sent to guest at `INDEX` 1.

.. Test case: `email -1`+
   Expected: No email sent as `INDEX` is out of bounds, error shown in command box.

.. Other incorrect commands to try: `email letters`, `emailLL`, `email 12.3`, etc. +
...
.. Other incorrect commands to try: `emailAll extracharacters`, `emailAllLL`, etc. +
   Expected: No email sent due to incorrect parameters.
...
.. Test case: `emailSpecific t/Veg`+
   Expected: If current filtered list is empty *or* there are no guests with tag `Veg`, no email will be sent and an error message displayed.

.. Other incorrect commands to try: `emailSpecific t/t/`, `emailSpecific t/@&@` etc. +
   Expected: No email sent due to incorrect parameters.
// end::emailappendix[]
