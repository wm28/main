...
[cols="30%,70%"]
|====
    ^.^| image:ProductWorkflow.png[] Diagram 1 - Product Workflow .^| Invités was developed as part of my team’s Software Engineering Project, and some of its *main features* include the ability to *send mass emails*, *keep track of payments*, as well as *keep track of attendance*. To add to this, by using a standardised format, the application is able to take in Comma Separated Values (CSV) files and *import* data for a particular event.

    Thanks to the complementary features my group implemented, we were able to
                                                   create a streamlined workflow for event planners and managers
                                                   , as depicted in *Diagram 1* on the left.
|====
...
=== Team Invités
The team that worked with me on this project, as well as the features they implemented, are listed below:
