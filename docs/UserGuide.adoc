...
[NOTE]
====
Undoable commands: those commands that modify the guest list's content (`add_guest`, `delete_guest`, `edit_guest`, `removeTag`, `addTag`, and `clear`) and the event details display's content('add_event', 'edit_event' and 'delete_event').
====
