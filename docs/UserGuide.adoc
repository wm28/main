...
Imports guests with data from a CSV specified CSV file. The structure for the values in the CSV file is predefined and can be found at <<csvguestlistformat,CSV Guest List Format>>. +

Format: `import FILE_PATH`

****
* `FILE_PATH` shall only be a relative (relative to where the application Jar file is located) or an absolute file path.
...
Exports guests' data to a specified CSV file. The structure for the values in the CSV file is predefined and can be found at <<csvguestlistformat,CSV Guest List Format>>. +
