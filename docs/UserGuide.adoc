...
Finds guests whose names, phone numbers and/or email addresses
...
The CSV guest entries which are badly formatted or those which corresponds to an already existing guest in the guest list, will trigger an import report window as shown in figure 1. The window will show the offending CSV guest entries with their associated error messages.

|====
| image:ImportReportWindow.png[] Figure 1 - ImportReportWindow
|====
...
[NOTE]
====
If no `FILE_PATH` is provided, guests will be exported using the default filename, `exportedGuestBook.csv`.
====
